...
@router.put("/qr-codes/{uuid}", response_model=QRCodeResponse)
...
@router.delete("/qr-codes/{uuid}", response_model=QRCodeResponse)
...
from app.database.database import get_db
from app.schemas import QRCodeCreate, QRCodeResponse
from app.curd_operation.qr_code_curd import create_qr_code, get_qr_code_by_uuid, get_qr_codes_by_user_id, update_qr_code, delete_qr_code
...
@router.get("/qr-codes/user/{user_id}", response_model=list[QRCodeResponse])
async def get_qr_codes_by_user_route(user_id: str, db: AsyncSession = Depends(get_db)):
    qr_codes = await get_qr_codes_by_user_id(db=db, user_id=user_id)
    return qr_codes
